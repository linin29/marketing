...
public class GlobalExceptionHandler {
	...
	public ModelAndView handleNotFound(NoHandlerFoundException exception, HttpServletRequest request, Model model) {
		logger.error(exception.getMessage());
		String redirectUrl = "dashboard/index"; // front
		String reqestPath = request.getRequestURI();
		if (reqestPath.indexOf("/admin") >= 0) {
	...
	@ResponseBody
	public String handleHttpMessageNotReadable(HttpMessageNotReadableException exception, HttpServletRequest request) {
	...
	public String handleInternalError(RuntimeException exception, HttpServletRequest request, Model model) {
		logger.error(exception.getMessage());
		String redirectUrl = "dashboard/index"; // front
		String reqestPath = request.getRequestURI();
		if (reqestPath.indexOf("/admin") >= 0) {
